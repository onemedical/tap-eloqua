...
def stream_export(
    ...
    while has_more:
        LOGGER.info(
            "{} - Paginating export results - offset: {}, limit: {}".format(
                stream_name, offset, bulk_page_size
            )
        )
...
def sync_bulk_obj(
    ...
    if last_sync_id:
        LOGGER.info(
            "{} - Resuming previous export: {}".format(stream_name, last_sync_id)
        )
        try:
            ...
            if e.response.status_code in [404, 410]:
                LOGGER.info(
                    "{} - Previous export expired: {}".format(stream_name, last_sync_id)
                )
            else:
                raise
    ...
    for meta in stream.metadata:
        if not meta["breadcrumb"]:
            ...
            meta["metadata"].get("selected", True)
            or meta["metadata"].get("inclusion", "available") == "automatic"
        ):
    ...
    if num_fields > 250:
        raise Exception(
            "{} - Exports can only have 250 fields selected. {} are selected.".format(
    ...
    _filter = (
        "'{{" + language_obj + "." + updated_at_field + "}}' >= '" + last_date + "'"
    )
    ...
    with metrics.job_timer("bulk_export"):
        ...
        while True:
            data = client.get(
                "/api/bulk/2.0/syncs/{}".format(sync_id), endpoint="export_sync_poll"
            )
            ...
            LOGGER.info(
                "{} - status: {}, sleeping for {} seconds".format(
    ...
    # Check record count
    data = client.get(
        "/api/bulk/2.0/syncs/{}/logs".format(sync_id), endpoint="export_sync_poll"
    )

    success_message = "Successfully exported members to csv file."
    export_success_log = next(
        (i for i in data.get("items", []) if i.get("message") == success_message), None
    )
    if export_success_log:
        ...
        if activity_type and record_count >= 5000000:
            raise ActivityExportTooLarge(
                "Export too large, retrying with smaller window."
            )
...
def sync_static_endpoint(
    client, catalog, state, start_date, end_date, stream_id, path, updated_at_col
):
...
def sync_activity_stream(
    ...
    while not finished:
        try:
            ...
            last_date_raw = get_bulk_bookmark(state, stream_name).get(
                "datetime", start_date
            )
            ...
            LOGGER.warn(ex)
            batch_end = last_date.add(
                seconds=(batch_end - last_date).total_seconds() / 2
            )
            if batch_end > end_date:
                batch_end = end_date
...
def sync(client, catalog, state, start_date, end_date, bulk_page_size):
    ...
    for bulk_object in BUILT_IN_BULK_OBJECTS:
        should_stream, last_stream = should_sync_stream(
            selected_streams, last_stream, bulk_object
        )
        if should_stream:
    ...
    for static_endpoint in static_endpoints:
        ...
        should_stream, last_stream = should_sync_stream(
            selected_streams, last_stream, stream_id
        )
        if should_stream:
